...
def ensure_notebook_module():
    # Always show the API key input and allow manual override of the environment key.
    env_key = os.getenv("OPENAI_API_KEY", "")
    api_key = st.text_input("OpenAI API Key", type="password", value=env_key)
    if api_key:
        os.environ["OPENAI_API_KEY"] = api_key

    notebook_path = Path(__file__).resolve().parent / "code.py"

    try:
        if notebook_path.exists():
            spec = importlib.util.spec_from_file_location("resume_screening_code", notebook_path)
            if spec and spec.loader:
                nb_module = importlib.util.module_from_spec(spec)
                sys.modules["resume_screening_code"] = nb_module
                spec.loader.exec_module(nb_module)
            else:
                raise ImportError(f"Could not load module spec for {notebook_path}")
        else:
            raise FileNotFoundError(f"Local notebook module not found at {notebook_path}")
